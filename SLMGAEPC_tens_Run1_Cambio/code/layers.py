...
class AttentionRec():
    ...
    def __init__(self, output_dim, num_support, name, dropout=0., act=tf.nn.sigmoid, initial_weights=None):
        self.num_nodes = output_dim#Numero total de nodos
        ...
        with tf.variable_scope(self.name + '_attW'):
            if initial_weights is not None:
                init = np.array(initial_weights).reshape([num_support, 1, 1])
                init = np.tile(init, (1, output_dim, output_dim))
...
class InnerProductDecoder():
    ...
    def __call__(self, inputs):
        with tf.name_scope(self.name):
            ...
            x = tf.matmul(inputs, self.vars['weights'])#Imput array de embedding de cada gen
