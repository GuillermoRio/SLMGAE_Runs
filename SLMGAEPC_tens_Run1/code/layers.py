...
class AttentionRec():
    ...
    def __init__(self, output_dim, num_support, name, dropout=0., act=tf.nn.sigmoid):
        ...
        with tf.variable_scope(self.name + '_attW'):
            self.attweights = tf.get_variable("attWeights", [self.num_support, self.num_nodes, self.num_nodes],initializer=tf.random_uniform_initializer(minval=0.9, maxval=1.1))
