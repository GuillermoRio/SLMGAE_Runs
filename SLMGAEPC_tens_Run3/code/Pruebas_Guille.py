...
adj_orig = sp.csr_matrix((np.ones(len(pos_edge)), (pos_edge[:, 0], pos_edge[:, 1])), shape=(654, 654))#332
...
for train_pos, test_pos in pos_edge_kf:
    ...
    adj = sp.csr_matrix((val, (row, col)), shape=(654, 654))
    ...
    slMapping = {}
    with open('../../SLMGAEPC_tens_Run2/data/gene_list_cohortes_MiaPaca_screening.csv', 'r') as inf:
        id = 0
        for line in inf:
    ...
    df.to_csv(f'Top_nonPred{name}.csv')
    name = name + 1
